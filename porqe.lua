...
pcall(function()
    ...
    if CoreGui:FindFirstChild("ERDEVA_KEY") then
        CoreGui:FindFirstChild("ERDEVA_KEY"):Destroy()
    end
end)
...
local function RequestServerTrial()
    ...
    if not ok or not res then
        return false, 0, "Failed to connect to trial server!"
    end
    local parseOk, data = pcall(function()
        return HttpService:JSONDecode(res)
    end)
...
LaunchKeyUI = function(isExpiredTrial)
    ...
    SubTitle.Text = isExpiredTrial and "Free trial has ended. Enter your license key:" or
                        "Enter license key from ERDEVA HUB Discord:"
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        if SubmitBtn.Active ~= false then
            SubmitKey(TextInput.Text)
        end
    end)
    TextInput.FocusLost:Connect(function(enter)
        if enter and SubmitBtn.Active ~= false then
            SubmitKey(TextInput.Text)
        end
    end)
end
...
StartMainScript = function(isTrialMode, trialTimeLeft)
    ...
    local StatsPage = MakeTab("Stats", 7)
    ...
    AddToggle(InfoPage, "Spoofing Name", "VisualName")
    ...
    local LabelTime = CreateStatCard(StatsPage, "ONLINE DURATION", "00h 00m 00s", C.Txt)
    local LabelCurRebirth = CreateStatCard(StatsPage, "CURRENT REBIRTH", "0", Color3.fromRGB(56, 189, 248))
    local LabelGainRebirth = CreateStatCard(StatsPage, "SESSION REBIRTHS GAINED", "+0", Color3.fromRGB(34, 197, 94))
    local LabelRateRebirth = CreateStatCard(StatsPage, "REBIRTH / HOUR", "0 / hr", Color3.fromRGB(234, 179, 8))
    local LabelSoldChicken = CreateStatCard(StatsPage, "CHICKENS SOLD", "0", Color3.fromRGB(239, 68, 68))
    local LabelRateChicken = CreateStatCard(StatsPage, "CHICKENS SOLD / HOUR", "0 / hr", Color3.fromRGB(168, 85, 247))
    ...
    ResetStatsBtn.Text = "RESET COUNTER"
    ...
    task.spawn(function()
        while IsRunning do
            ...
            if LabelTime and LabelTime.Parent then
                ...
                LabelRateRebirth.Text = stats.RebirthPerHour .. " / hr"
                LabelSoldChicken.Text = stats.ChickensSold .. " chickens"
                LabelRateChicken.Text = stats.ChickensPerHour .. " / hr"
            end
            task.wait(1)
        end
    end)
